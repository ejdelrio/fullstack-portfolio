...
class PrismSnip extends React.Component{
  constructor(props) {
    super(props);
    this.state = {
      node: 'hidden',
  ...
  render() {
    switch(this.props.tech) {
      case 'Node.js':
        return this.setState({node: ''});
        break;
      case 'React.js / Redux':
        return this.setState({react: ''});
        break;
      case 'MongoDB':
        return this.setState({mongo: ''});
        break;
      case 'AWS':
        return this.setState({aws: ''});
        break;
      default:
        null;
    }
    return(
      <div>
        ...
        <div>
          <p>
            Reacts one way data flow, complimented by Reduxs state managment lends way to the development of incredibly performant applications. Reacts component architecture is akin to a digital playground. Its by far my favorite framework to work with. The code below displays a React reusable React component that take several props, one being an object constructor for a trie search tree. The tree is than build using an array of strings. Any text input is passed into the tree to search, the the results are rendered below.
          </p>
          <pre><code className='language-javascript'>
          {`
          import React from 'react';
